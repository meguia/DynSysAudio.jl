...
# ╔═╡ c6af3313-eb0a-4d48-b6ef-9b04168c1491
...
soundcard = PortAudioStream(sdev[3],0,2)
...
ode_source = ODESource(Float64, thomas!, 44100, 5.0, [1.0;1.1;-0.01],[0.2,0.2],1:3);

# ╔═╡ ec64fe3c-bf0e-4dd0-a625-61fb98ec0f8f
ode_source.nchannels

# ╔═╡ f99e2ea3-dba7-4068-94ea-7d47c56e2df8
...
md"""
gain $(@bind g Slider(0:0.01:1.0,default=0.5;show_value=true)) \
a $(@bind a Slider(0.0:0.01:5.0,default=1.0;show_value=true)) \
b $(@bind b Slider(0.0:0.001:1.0,default=0.2;show_value=true)) \
Δt $(@bind Δt Slider(0.00001:0.001:1.0,default=0.11;show_value=true)) \
"""
...
# ╔═╡ 6cc6ad43-0298-4ebd-9d1d-e7445d09b57f
@pipe read(ode_source, 0.3u"s") |> send_osc(sock1,_) |> write(soundcard, _)

# ╔═╡ ff7017bb-d226-4030-a5e0-1d8f1320105d
mutable struct Comm_control{T}
	sender_active::Bool
	sock::UDPSocket
	ode_sender::Task
	oscMessage::OscMsg
...
# ╔═╡ db1b2264-4600-4a8f-9170-ac3c72d84346
...
# ╔═╡ 44abe12e-6670-4715-9b55-b18a23601c2a
function source_create(eltype, system::Function, timescale::Number, start_point::Array, pars::Array)
    ...
    ODE_source{eltype}(Float64(timescale), time, dt, uini, pars, problem)
end

# ╔═╡ bd3700a8-f993-492e-ba01-b9c2d8e6b7a3
...
# ╔═╡ e3bc2ffa-56b8-4f37-b9d5-28e0fe643636
function init_odes()
	global ode
	
	ode = source_create(Float64, thomas!, 500.0, [1.0;1.1;-0.01],[0.2,0.2])
	ode
	#ode_source = ODESource(Float64, thomas!, 44100, 5.0, params,[0.2,0.2],2:3);
end

# ╔═╡ 8977cddb-7fbc-4144-8795-b6ca08006ef1
if !@isdefined(comm_control)
	comm_control = Comm_control{eltype}(false,UDPSocket(), Task(""),OscMsg("/none","d",0))
end

# ╔═╡ 9e6b85e1-345a-4519-b095-45ff33a67a2a
ode_stream = Threads.@spawn begin
    while ode_source.gain>0.0
        @pipe read(ode_source, 0.3u"s") |> send_osc(comm_control.sock,_) 
    end
end

# ╔═╡ 26af5e5c-eb47-49f4-aecb-b7da1d74106d
@pipe read(ode_source, 0.3u"s") |> send_osc(comm_control.sock,_) 

# ╔═╡ 0a5b7e6e-2b66-46e4-8380-58ebbcd41549
begin
	if !@isdefined(ode)
		@warn "Defining ode"
		init_odes()
...
# ╔═╡ 629cbdc5-92a9-47a9-959c-07df2d4dfafa
ode

# ╔═╡ aefbe74c-eec3-454b-bd9e-6caeecbe722d
md"""
Enable OSC sender $(@bind enable_sender CheckBox(default=false))
"""

# ╔═╡ fb97eb53-b7aa-4e7f-99d8-a21ffb94bb33
begin
	if enable_sender
		if comm_control.sender_active == false
			init_odes()
			comm_control.sender_active = true
			comm_control.sock = UDPSocket()
			comm_control.ode_sender = Threads.@spawn begin
				while comm_control.sender_active
					u = source_step!(ode,1)
					msg1 = OpenSoundControl.message("/ode", "ddd",u[1],u[2],u[3])
					send(comm_control.sock, ip"127.0.0.1", 7779, msg1.data)
					sleep(0.01) # borrar esto esta solo para monitorear
				end
			end
		end
	else
		if comm_control.sender_active
			comm_control.sender_active = false
		end
	end
	@warn comm_control.sender_active
end

# ╔═╡ e56607b9-cc5d-4e02-8d42-b45dfeeb66a4
begin
	println(comm_control.sender_active)
	println(enable_sender)
	println(comm_control.ode_sender)
end

# ╔═╡ 9adc7782-b6b6-4a22-914f-29dcaf39cc34
begin
	global ode
	u2 = source_step!(ode,1)
	msg1 = OpenSoundControl.message("/ode", "ddd",u2[1],u2[2],u2[3])
	send(comm_control.sock, ip"127.0.0.1", 7779, msg1.data)
	u2
end

# ╔═╡ f2ae8a7c-3e10-4d38-9c90-d75116815d0a
uf = source_step!(ode,1)
